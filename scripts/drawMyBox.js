...
var groupsData = {
  // This will be retrieved by a call to the back-end
  groups: [
    {
      id: "group1",
      name: "Thickener 1",
...
function createDraggableGroup(data, fillColor) {
  ...
  group.text.move(
    data.x + (data.w - bbox.width) / 2,
    data.y + (data.h - bbox.height) / 2
  );
...
function drawLineAndArrow(group) {
  ...
  const { lineStartX, lineStartY, lineEndX, lineEndY } = calculateLineEnds(
    group.data
  );
  ...
  var newArrow = group
    .polygon(`0,0 0,${arrowHeight} ${arrowWidth},0 0,-${arrowHeight}`)
...
function calculateLineEnds(data) {
  ...
  if (data.destGroup === null) {
    ...
    const destGroupData = groupsData.groups.find(
      (g) => g.id === data.destGroup
    );
    if (destGroupData) {
